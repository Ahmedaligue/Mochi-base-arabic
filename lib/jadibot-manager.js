...
export async function iniciarJadibot(conn, m, userJid, usePairing = false, isAutoStart = false) {
    ...
    if (!fs.existsSync('./jadibots_sesiones')) fs.mkdirSync('./jadibots_sesiones', { recursive: true });
    ...
    if (!jadibotSess.has(userJid)) {
        jadibotSess.set(userJid, { 
            status: 'initializing', 
            number: numeroLimpio,
            config: { welcome: true, only_private: false } // Nuevos ajustes
        });
    }
    ...
    client.ev.on('messages.upsert', async (chat) => {
        ...
        // --- INTEGRACIÓN DE FILTROS DEL PANEL ---
        const settings = jadibotSess.get(userJid)?.config || {};
        // Si "Solo Privados" está activo y el mensaje viene de un grupo, ignoramos.
        if (settings.only_private && msg.isGroup) return;
...
export async function stopJadibot(jid) {
    if (subBots.has(jid)) {
        ...
        // Borramos carpeta para que no se autoinicie un bot eliminado
        if (fs.existsSync(authPath)) fs.rmSync(authPath, { recursive: true, force: true });
